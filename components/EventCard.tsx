...
import { CalendarDays, Check, CircleArrowRight, Divide, LoaderCircle, MapPin, PencilIcon, StarIcon, Ticket, XCircle } from 'lucide-react'
...
const EventCard = ({eventId}:{eventId:Id<"events">}) => {
  ...
  const renderQueuePosition=()=>{
    ...
    return (
      <div className='flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200'>
        <div className='flex items-center'>
          ...
          <span className='text-gray-400'>Event is sold out</span>
   ...
   if(queuePosition.position===2){
        return (
          <div className='flex flex-col lg:flex-row items-center justify-between p-3 bg-amber-50 border-amber-100'>
            <div className='flex items-center'>
  ...
  const renderTicketStatus=()=>{
    ...
    if(userTicket)
    {
      <div className='mt-4 flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-100'>
        ...
        <button onClick={()=>router.push(`/tickets/${userTicket._id}`)} className='text-sm bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-full font-medium shadow-sm transition-colors duration-200 flex items-center gap-1'>
         View Your Ticket
        </button>
      </div>
    }
    if(queuePosition)
    {
      return (
        <div className='mt-4'>
          {/* /if queue position is 1st */}
          {queuePosition.status==="offered"&&(
            <PurchaseTicket eventId={eventId}/>
          )}
          {/* if queue position is beyond 1 */}
          {renderQueuePosition()}
          {/* if queue position is expiered that is ticket is not offered */}
  ...
  return (
    <div onClick={()=>router.push(`/event/${eventId}`)} className={`bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 cursor-pointer overflow-hidden relative ${
      isPastEvent ? "opacity-75 hover:opacity-100" : ""
    }`}>
       {/* Event Image */}
     {imageUrl&&  ( <div className="relative w-full h-48">
          <Image
            src={imageUrl}
            alt={event.name}
            fill
       ...
       <p className='mt4 text-gray-600 text-sm line-clamp-2'>{event.description}</p>
      {/* stop propagation will stop the useRouter functionality for this particular part and you can add your own navigation if this div is clicked  */}
