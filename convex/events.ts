...
export const getUserTickets=query({
  ...
  handler:async (ctx,{userId})=>{
    const tickets=await ctx.db.query("tickets").withIndex("by_user",(q)=>q.eq("userId",userId))
    .collect()
...
export const checkAvailability=query({
  ...
  handler:async (ctx,{eventId})=>{
    const event=await ctx.db.get(eventId)
    if(!event)throw new Error("event not found")
      const purchasedCount=await ctx.db.query("tickets").withIndex("by_event",(q)=>q.eq("eventId",eventId)).collect()
    .then((tickets)=>tickets.filter((t)=>t.status===TICKET_STATUS.VALID||t.status===TICKET_STATUS.USED).length)

    const now=Date.now()
    const activeOffers=await ctx.db.query("waitingList").withIndex("by_event_status",(q)=>q.eq("eventId",eventId).eq("status",WAITING_LIST_STATUS.OFFERED)).collect()
...
export const joinWaitingList=mutation({
  ...
  handler:async(ctx,{eventId,userId})=>{
    const exitingEntry=await ctx.db.query("waitingList").withIndex("by_user_event",(q)=>q.eq("userId",userId).eq("eventId",eventId))
    .filter((q)=>q.neq(q.field("status"),WAITING_LIST_STATUS.EXPIRED)).first()
    if(exitingEntry)
    {
      throw new Error("Already waiting for this event")
    }
    const event=await ctx.db.get(eventId)
    if(!event)
      throw new Error("event not found")
    ...
    if(available)
    {
      ...
      await ctx.db.insert("waitingList",{
        eventId,userId,status:WAITING_LIST_STATUS.WAITING
      })
    }
    return{
      success:true,
      status:available?WAITING_LIST_STATUS.OFFERED:WAITING_LIST_STATUS.WAITING,
      message:available?`Ticket offered-You have ${DURATIONS.TICKET_OFFER/(60*1000)}minutes to purchase`:"added to waiting list-you will be notified when a ticket becomes available"
    }
  }
})
